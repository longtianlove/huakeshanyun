...
public class DateUtil {
	...
	/**
	 * 给定时间，增加月份，判断是否超过当前时间 没有超过当前时间 返回true ，超过返回false
	 * 
	 * @param time
	 * @param num
	 * @return
	 */
	public static boolean isBeforeNowDay(Date time, int month_num) {
		Date now = new Date();
		Calendar cal = Calendar.getInstance();
		cal.setTime(time);
		cal.add(Calendar.MONTH, month_num);
		Date overTime = cal.getTime();
		if (overTime.before(now)) {
			return true;
		}
	...
	public static boolean timeDifferenceByDay2(Date time, int minute_num) {
		Date now = new Date();
		Calendar cal = Calendar.getInstance();
		cal.setTime(time);
		cal.add(Calendar.MINUTE, minute_num);
		Date overTime = cal.getTime();
		if (overTime.before(now)) {
			return true;
		}
	 ...
	 * @return
	 */
	public static Date addDay(String date, int num) {
		Calendar calendar = Calendar.getInstance();
		try {
	 ...
	 * @return
	 */
	public static String addMonthtoString(String date, int num) {
		Calendar calendar = Calendar.getInstance();
		SimpleDateFormat fmt = new SimpleDateFormat("yyyy-MM-dd");
		try {
			calendar.setTime(fmt.parse(date));
	...
	public static void main(String[] args) {
		...
		/*
		 * boolean flag = DateUtil.isBeforeNowDay(DateUtil.fomatDate("2019-2-25"), 1);
		 * System.out.println("当前状态:" + flag);
		 */
//		System.out.println(addDay("2019-04-22", 2));
	}
