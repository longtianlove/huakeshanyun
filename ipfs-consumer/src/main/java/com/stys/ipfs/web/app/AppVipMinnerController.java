...
public class AppVipMinnerController extends AppController {
	...
	public @ResponseBody AppResultInfo<?> becomeVipMiner(Integer userId) {
		...
		Double outNumber = Double
				.parseDouble(BigDecimalUtils.multiply(TbProduct.getProductPreferentialPrice().toString(), "4"));
	...
	public @ResponseBody AppResultInfo<?> getVipGiftCoinTransfer(Integer userId, Double balance) {
		TbVipminer tbvipminer = itbVipminerService.selectOne(new EntityWrapper<TbVipminer>().eq("user_id", userId));
		Double giftCoin = tbvipminer.getGiftCoin();
		Double limitCoin = tbvipminer.getLimitCoin();
		if (giftCoin < balance) {
		...
		Double coin = userassets.getCoin();
		...
		if (itbUserAssetsService.updateById(userassets)) {
			tbvipminer.setGiftCoin(Double.parseDouble(newgiftCoin));
			tbvipminer.setLimitCoin(Double.parseDouble(newlimitCoin));
