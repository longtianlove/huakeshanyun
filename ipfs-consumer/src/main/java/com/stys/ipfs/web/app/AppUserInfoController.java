...
public class AppUserInfoController extends AppController {
	...
	@RequestMapping("/registerToCode")
	public @ResponseBody AppResultInfo<?> registerToCode() throws WriterException, IOException {

		List<AppUserinfo> list = iappUserinfoService.selectList(new EntityWrapper<AppUserinfo>());
		for (AppUserinfo appUserinfo : list) {
			AppUser user = iappUserService.selectOne(new EntityWrapper<AppUser>().eq("id", appUserinfo.getUserId()));

			String shareUrl = idicService.getDicData("网站地址", "邀请码登录地址").getValue1()
					+ "/invite/openshare?invitation_code=" + appUserinfo.getCode();
			try {
				String PATH = TwoDimensionCode.getEncode(shareUrl, user.getPhone());
				appUserinfo.setCodePath(PATH);
				iappUserinfoService.updateById(appUserinfo);
		...
		List<TbUsdtUser> usdtlist = itbUsdtUserService.selectList(new EntityWrapper<TbUsdtUser>());

		for (TbUsdtUser tbUsdtUser : usdtlist) {

			int usdtid = tbUsdtUser.getUsdtId();

			TbUsdt usdt = itbUsdtService.selectById(usdtid);
			if (null != usdt) {

				String addressImgpath = TwoDimensionCode.makeUSDTCodeImg(usdt.getUsdtAddr());
				usdt.setAddressImgpath(addressImgpath);
				if (itbUsdtService.updateById(usdt)) {

				}
			}
		}

		return new AppResultInfo<>("success");
	}
...
//		}
		...
		;
		appuser.setDicId(idicService.getDicData("用户级别", "F0").getId());// 初始用户等级未定
		appuser.setState(1);
		if (iappUserService.insert(appuser)) {
			...
			appuserinfo.setNickname("ID" + RandomUtil.getNumber(6));
			appuserinfo.setInvitationCode(invitationCode);
			// 这里增加邀请表信息
			String invitcode = RandomUtil.generateRandomStr(5);
			String shareUrl = idicService.getDicData("网站地址", "邀请码登录地址").getValue1()
					+ "/invite/openshare?invitation_code=" + invitcode;
			appuserinfo.setCode(invitcode);
			try {
				appuserinfo.setCodePath(TwoDimensionCode.getEncode(shareUrl, phone));
			} catch (Exception e) {
				logger.error("二维码生成出错");
			}
			iappUserinfoService.insert(appuserinfo);
			// 初始化用户累计
			TbUserExperience entity = new TbUserExperience();
			entity.setCreateTime(new Date());
			entity.setExperience(0);
			entity.setUserId(userId);
			itbUserExperienceService.insert(entity);
			// USDT 生成
			// 添加邀请关系
			...
			userassets.setCoin(0d);
			userassets.setGiftCoin(0d);
			userassets.setLimitCoin(0d);
	...
	public @ResponseBody AppResultInfo<?> loginForUser(String phone, String sessionId, ServletRequest request,
			ServletResponse response, String code) {
		Map<String, Object> result = this.returnMap();
		AppUser appuser = iappUserService.selectOne(new EntityWrapper<AppUser>().eq("phone", phone));
		if (appuser == null) {
			logger.info("=======用户不存在========");
...
//		验证验证码
//		if(!"8972".equals(code)&&!"15910423974".equals(phone)) {
//			if( verifySession(phone, sessionId, request, response, code,result)!=AppConstant.STATUE_200) {
//				return new AppResultInfo<>(result);
//			}
//		}
		...
		appLoginLog.setUserId(appuser.getId());
		appLoginLog.setPhone(phone);
		appLoginLog.setCreateTime(new Date());
		iappLoginLogService.insert(appLoginLog);
		logger.info("登录成功。。。。。。。。。。。。。");
		return new AppResultInfo<>(result);
	}
	 ...
	 * 设置支付密码
	 * 
	 * @api {post} /tbr/setPaymentCode 设置支付密码
	 * @apiGroup userBase
	 * @apiDescription 详细描述：设置支付密码
	 * @apiParam {String} paymentCode 支付密码
	 * @apiParam {Integer} userId 用户Id
	 * @apiVersion 0.1.0
	 * @apiSampleRequest http://127.0.0.1/tbr/setPaymentCode
	 */
	@RequestMapping("/setPaymentCode")
	public @ResponseBody AppResultInfo<?> setPaymentCode(String paymentCode, Integer userId) {
		if (null == userId || null == paymentCode) {
