   	...
   	public @ResponseBody ResultInfo<?> personBuy(Integer id) {
		...
		synchronized (newtbUserAssets) {
			...
			Double outNumber = Double.parseDouble(BigDecimalUtils.multiply(tbProduct.getProductPreferentialPrice().toString(),
					"5"));
			Double newLimit=tbVipminer.getLimitCoin()+outNumber;
			tbVipminer.setLimitCoin(newLimit);
			Double befor=newtbUserAssets.getCoin();
			// 用户余额
			Double balance = Double.parseDouble(BigDecimalUtils.subtract(befor.toString(),
