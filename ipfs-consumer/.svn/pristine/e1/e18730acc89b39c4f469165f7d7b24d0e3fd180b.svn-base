...
public class TbUserAssetsController extends BaseController {
	...
	public @ResponseBody ResultInfo<Integer> count() { 
    	Map<String, Object> map =itbUserAssetsService.selectMap(new EntityWrapper<TbUserAssets>().setSqlSelect("sum(coin) as totalCoin"));
    	BigDecimal totalCoin=new BigDecimal("0");
    	if (map != null && map.size() > 0) {
    		totalCoin = (BigDecimal) map.get("totalCoin");
    	}
    	return new ResultInfo<>(totalCoin.toString());
	}
}
