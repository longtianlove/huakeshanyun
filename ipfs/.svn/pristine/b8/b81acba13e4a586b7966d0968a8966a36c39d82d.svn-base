...
/**
 * <p>
 * Encode Data into MultiSig transaction outputs e.g. for Omni Protocol Class B
 * transactions.
 * </p>
 *
 * @author msgilligan
...
public class EncodeMultisig {
	 ...
	 *
	 * @param redeemingKey key that can be used redeem transaction output (1 of n
	 *                     multisig)
	...
	public Transaction encode(ECKey redeemingKey, byte[] data) {
		...
		for (List<ECKey> group : keysByOutput) {
			...
			Script script = ScriptBuilder.createMultiSigOutputScript(1, redeemableGroup); // 1 of redeemableGroup.size()
																							// multisig
//            Script script = ScriptBuilder.createS(1,redeemableGroup);
