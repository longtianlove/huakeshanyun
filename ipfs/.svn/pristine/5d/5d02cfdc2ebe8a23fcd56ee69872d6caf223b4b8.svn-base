...
public class TbProductServiceImpl extends ServiceImpl<TbProductMapper, TbProduct> implements ITbProductService {
	 ...
	 * @return
	 */
	private AppUser getInviterAndAddExperience(AppUser user, Float price, Integer type, Float percent, int first) {
		...
		if (!StringUtils.isEmpty(tbinvite)) {
			...
			if (appuser.getDicId().intValue() != s3) {
				...
				if (user.getDicId().intValue() < appuser.getDicId().intValue()) {
					addProfit(user.getId(), appuser.getId(), price, pre - percent);
				}
			} else if (appuser.getDicId().intValue() == s3) {
				first++;
				// 当前用户达到S3 ,并且他的下级也没有达到S3
				if (user.getDicId().intValue() != s3) {
					addProfit(user.getId(), appuser.getId(), price, pre - percent);
				} else if (user.getDicId().intValue() == s3 && first == 2) {
					// 当前用户达到S3 ,并且他的下级达到S3
					addProfit(user.getId(), appuser.getId(), price, 0.01f);
				}
			}

			return getInviterAndAddExperience(appuser, price, type, pre, first);
		}
		return null;
	}
