...
public class AppUserinfoServiceImpl extends ServiceImpl<AppUserinfoMapper, AppUserinfo> implements IAppUserinfoService {
	...
	public void upGared() {
		...
		for (AppUser appUser : list) {
			...
			//获取当前用户的所有一级邀请人
			List<TbInvite> inviteList = itbInviteService.selectList(new EntityWrapper<TbInvite>().eq("tb_iviter", userId));
			//确保用户最少拥有2条线     单条线路用户不能升级
			List <Integer> arr=new ArrayList<>();
			if(inviteList!=null&&inviteList.size()>1) {
				for (TbInvite invite : inviteList) {
					TbUserExperience userExperience= itbUserExperienceService.selectOne(new EntityWrapper<TbUserExperience>().eq("user_id", invite.getTbIvitee()));
		       ...
		       //获取当前用户的累计
		      TbUserExperience experience= itbUserExperienceService.selectOne(new EntityWrapper<TbUserExperience>().eq("user_id", userId));
