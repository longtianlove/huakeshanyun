...
import com.stys.ipfs.test.CurrencyID;
import com.stys.ipfs.test.Ecosystem;
import com.stys.ipfs.test.OmniDivisibleValue;
import com.stys.ipfs.test.OmniValue;
import com.stys.ipfs.test.PropertyType;
...
public class RawTxBuilder {
	...
	public String createMySimpleSendHex(CurrencyID currencyId, OmniValue amount, String toaddress) {
		String rawTxHex = String.format("00000000%08x%016x", currencyId.getValue(), amount.getWillets(), toaddress);
		return rawTxHex;
	}
	...
	public String createDexSellOfferHex(CurrencyID currencyId, OmniDivisibleValue amountForSale, Coin amountDesired,
		...
		String rawTxHex = String.format("00010014%08x%016x%016x%02x%016x%02x", currencyId.getValue(),
				amountForSale.getWillets(), amountDesired.value, paymentWindow, commitmentFee.value, action);
		return rawTxHex;
	}
	 ...
	 * @return The hex-encoded raw transaction
	 */
	public String createMetaDexSellOfferHex(CurrencyID currencyForSale, OmniValue amountForSale,
			CurrencyID currencyDesired, OmniValue amountDesired, Byte action) {
		String rawTxHex = String.format("00000015%08x%016x%08x%016x%02x", currencyForSale.getValue(),
				amountForSale.getWillets(), currencyDesired.getValue(), amountDesired.getWillets(), action);
		return rawTxHex;
	}

	/**
	 * Creates a hex-encoded raw transaction of type 22: "purchase tokens with
	 * bitcoins".
	 * 
	...
	public String createAcceptDexOfferHex(CurrencyID currencyId, OmniValue amount) {
		String rawTxHex = String.format("00000016%08x%016x", currencyId.getValue(), amount.getWillets());
		return rawTxHex;
	}

	/**
	 * Creates a hex-encoded raw transaction of type 50: "create a property with
	 * fixed supply".
	 * 
	 * @param ecosystem          main or test ecosystem
	 * @param propertyType       divisible or indivisible
	 * @param previousPropertyId an identifier of a predecessor token (0 for new
	 *                           tokens)
	 * @param category           a category for the new tokens (can be "")
	 * @param subCategory        a subcategory for the new tokens (can be "")
	 * @param label
	 * @param website            an URL for further information about the new tokens
	 *                           (can be "")
	 * @param info
	...
	public String createPropertyHex(Ecosystem ecosystem, PropertyType propertyType, Long previousPropertyId,
			String category, String subCategory, String label, String website, String info, OmniValue amount) {
		String rawTxHex = String.format("00000032%02x%04x%08x%s00%s00%s00%s00%s00%016x", ecosystem.getValue(),
				propertyType.getValue(), previousPropertyId, toHexString(category), toHexString(subCategory),
				toHexString(label), toHexString(website), toHexString(info), amount.getWillets());
		return rawTxHex;
	}

	/**
	 * Creates a hex-encoded raw transaction of type 51: "create a property via
	 * crowdsale with variable supply".
	 * 
	 * @param ecosystem          main or test ecosystem
	 * @param propertyType       divisible or indivisible
	 * @param previousPropertyId an identifier of a predecessor token (0 for new
	 ...
	 *                           (can be "")
	 * @param info
	 * @param propertyDesired    the identifier of a token eligible to participate
	 *                           in the crowdsale
	 * @param tokensPerUnit      the amount of tokens granted per unit invested in
	 *                           the crowdsale
	 * @param deadline           the deadline of the crowdsale as Unix timestamp
	 * @param earlyBirdBonus     an early bird bonus for participants in percent per
	 *                           week
	 * @param issuerBonus        a percentage of tokens that will be granted to the
	 *                           issuer
	 * @return The hex-encoded raw transaction
	 */
	public String createCrowdsaleHex(Ecosystem ecosystem, PropertyType propertyType, Long previousPropertyId,
			String category, String subCategory, String label, String website, String info, CurrencyID propertyDesired,
			Long tokensPerUnit, Long deadline, Byte earlyBirdBonus, Byte issuerBonus) {
		String rawTxHex = String.format("00000033%02x%04x%08x%s00%s00%s00%s00%s00%08x%016x%016x%02x%02x",
				ecosystem.getValue(), propertyType.getValue(), previousPropertyId, toHexString(category),
				toHexString(subCategory), toHexString(label), toHexString(website), toHexString(info),
				propertyDesired.getValue(), tokensPerUnit, deadline, earlyBirdBonus, issuerBonus);
		return rawTxHex;
	}

	/**
	 * Creates a hex-encoded raw transaction of type 53: "close a crowdsale
	 * manually".
	 * 
	...
	/**
	 * Creates a hex-encoded raw transaction of type 54: "create a managed property
	...
	public String createManagedPropertyHex(Ecosystem ecosystem, PropertyType propertyType, Long previousPropertyId,
		...
		String rawTxHex = String.format("00000036%02x%04x%08x%s00%s00%s00%s00%s00", ecosystem.getValue(),
				propertyType.getValue(), previousPropertyId, toHexString(category), toHexString(subCategory),
				toHexString(label), toHexString(website), toHexString(info));
		return rawTxHex;
	}

	/**
	 * Creates a hex-encoded raw transaction of type 55: "grant tokens for a managed
	 * property".
	 * 
	...
	public String createGrantTokensHex(CurrencyID currencyId, OmniValue amount, String memo) {
		String rawTxHex = String.format("00000037%08x%016x%s00", currencyId.getValue(), amount.getWillets(),
				toHexString(memo));
		return rawTxHex;
	}

	/**
	 * Creates a hex-encoded raw transaction of type 56: "revoke tokens of a managed
	 * property".
	 * 
	...
	public String createRevokeTokensHex(CurrencyID currencyId, OmniValue amount, String memo) {
		String rawTxHex = String.format("00000038%08x%016x%s00", currencyId.getValue(), amount.getWillets(),
				toHexString(memo));
		return rawTxHex;
	}

	/**
	 * Creates a hex-encoded raw transaction of type 70: "change manager of a
	 * managed property".
	 * 
	...
	/**
	 * Convert a hexadecimal string representation of binary data to byte array.
	 *
