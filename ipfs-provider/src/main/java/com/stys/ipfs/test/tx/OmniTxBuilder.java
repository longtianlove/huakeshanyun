...
import com.stys.ipfs.test.net.OmniNetworkParameters;
...
public class OmniTxBuilder {
	 ...
	 * </p>
	 *
	 * @param redeemingKey Public key used for creating redeemable multisig data
	 *                     outputs
	 * @param refAddress   (optional) Omni reference address (for the reference
	 *                     output) or null
	 ...
	 * @param unspentOutputs A collection of unspent outputs for funding the
	 *                       transaction
	 ...
	 */
	public Transaction createSignedOmniTransaction(ECKey fromKey, Collection<TransactionOutput> unspentOutputs,
			Address refAddress, byte[] payload) throws InsufficientMoneyException {
		Address fromAddress = fromKey.toAddress(netParams);

		Transaction tx = createOmniTransaction(fromKey, refAddress, payload);

		// Add all UTXOs for fromAddress as unsigned inputs, so fee calculator can use
		// length
		for (TransactionOutput output : unspentOutputs) {
			tx.addInput(output);
		}

		makeChangeOutput(tx, fromAddress, sum(unspentOutputs)); // Calculate fees/change and, if any, return to the
																// fromAddress
		...
		for (int i = 0; i < tx.getInputs().size(); i++) {
			...
			TransactionSignature signature = tx.calculateSignature(i, fromKey, scriptPubKey, Transaction.SigHash.ALL,
					false);
	...
	/**
	 * Create an unsigned Omni transaction, with unsigned inputs in a bitcoinj
	 * Transaction object
	 *
	 * @param fromKey    Private key/address to send from and receive change to
	 ...
	 * @throws InsufficientMoneyException Not enough bitcoin for fees
	 */
	public Transaction createUnsignedOmniTransaction(ECKey fromKey, Collection<TransactionInput> inputs,
			Address refAddress, byte[] payload) throws InsufficientMoneyException {
		Address fromAddress = fromKey.toAddress(netParams);
	 ...
	 * @param unspentOutputs A collection of unspent outputs for funding the
	 *                       transaction
	 * @param toAddress      The Omni reference address (for the reference output,
	 ...
	 * @return Signed and ready-to-send Transaction
	 */
	public Transaction createSignedSimpleSend(ECKey fromKey, Collection<TransactionOutput> unspentOutputs,
			Address toAddress, CurrencyID currencyID, OmniValue amount) throws InsufficientMoneyException {
	 ...
	 * @throws InsufficientMoneyException Not enough bitcoin for fees
	 */
	public Transaction createUnsignedSimpleSend(ECKey fromKey, Collection<TransactionInput> inputs, Address toAddress,
			CurrencyID currencyID, OmniValue amount) throws InsufficientMoneyException {
		String txHex = builder.createSimpleSendHex(currencyID, amount);
		byte[] payload = RawTxBuilder.hexToBinary(txHex);
	 ...
	 * @throws InsufficientMoneyException Not enough bitcoin for fees
	 */
	private Transaction makeChangeOutput(Transaction tx, Address changeAddress, long totalInputAmount)
