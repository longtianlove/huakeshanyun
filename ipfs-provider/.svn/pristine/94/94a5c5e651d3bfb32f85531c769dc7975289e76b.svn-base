...
public class TbUsdtServiceImpl extends ServiceImpl<TbUsdtMapper, TbUsdt> implements ITbUsdtService {
						...
						if (confirmations > 0) {
							...
							if (tbTransferUsdtList.size() == 0) {
								...
								if (null != tbUsdtVO && null != tbUsdtVO.getUserId()) {
									...
									tbTransferUsdt.setUserId(user_id);
	 ...
	 * @param user_id
	 */
	private void updateAssist(String referenceaddress, TbTransferUsdt tbTransferUsdt, BigDecimal balance,
			Integer user_id) {
		...
		synchronized (tbUserAssets) {
			...
			// 增加余额
			String new_coin = BigDecimalUtils.add(String.valueOf(coin), String.valueOf(new_balance));
	 ...
	 * @throws Throwable
	 */
	private boolean transferTotalAssets(String referenceaddress, TbTransferUsdt tbTransferUsdt, BigDecimal balance,
			Integer user_id, String mainAddress) {
		try {

			String balance_str = balance.setScale(6, BigDecimal.ROUND_HALF_DOWN).toString();
			String totle_txid = CoinUtils.getInstance().send(referenceaddress, mainAddress, 31,
					Double.valueOf(balance_str), mainAddress);
			if (null != totle_txid) {
