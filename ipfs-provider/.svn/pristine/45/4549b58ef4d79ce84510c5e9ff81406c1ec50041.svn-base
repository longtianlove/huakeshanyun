...
public interface TbUserBuyLogMapper extends BaseMapper<TbUserBuyLog> {
	...
	@Select("<script> SELECT  a.* ,b.nickname,c.phone  FROM tb_user_buy_log a JOIN sys_app_userinfo b ON a.user_id =b.user_id "
			+ "join sys_app_user c on c.id=a.user_id  where 1=1  "
