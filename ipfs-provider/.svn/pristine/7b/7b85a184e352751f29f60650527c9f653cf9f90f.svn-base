...
public interface AppUserinfoMapper extends BaseMapper<AppUserinfo> {
	...
	@Select("<script> SELECT a.*,b.*,c.*,   IFNULL(d.personal_status, 0) AS personal_status, f.address_imgpath as usdtImgpath, f.usdt_addr, g.name AS lv "
			+ ",IF(IFNULL(h.user_id,0), 1,0) AS vipminer_status , fm.account FROM sys_app_user a "
			...
			+ "	JOIN tb_usdt f on f.id=e.usdt_id  	 "
			+ "  left join  tb_vipminer  h on h.user_id=a.id  LEFT JOIN tb_feimauser fm ON fm.user_id = a.id "
