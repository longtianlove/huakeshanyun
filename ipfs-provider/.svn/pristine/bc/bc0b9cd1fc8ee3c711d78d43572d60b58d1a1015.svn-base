...
public interface TbAssetsDetailMapper extends BaseMapper<TbAssetsDetail> {
		 ...
		 * 	查询账务明细    包括
		 * 
		 *   38  usdt 充值   43 飞码通充值   47 线下充值  33金币提现   24 金币划转  63飞码通提现    62 USDT提现
		 * @param page
		 * @param userId
		 * @return
		 */
	...
	@Select("<script>SELECT  * FROM ( SELECT a.create_time ,a.in_or_out,a.amount,b.phone AS myaccont,c.phone AS otheraccont,a.type FROM "
			+ " tb_assets_detail a JOIN sys_app_user b ON a.user_id=b.id LEFT JOIN sys_app_user c ON a.sun_user_id=c.id WHERE a.type IN(38,43,47,33,24,63,62) "
