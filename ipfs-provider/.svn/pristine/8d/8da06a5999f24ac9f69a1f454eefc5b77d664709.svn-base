...
/**
 * <p>
 * Numeric value for a quantity of Omni tokens - base class for OmniDivisible
 * and OmniIndivisible subclasses. Known as "Number of Coins" in the Omni
 * Protocol Specification.
 * </p>
 *
 * <p>
 * The internal representation is a <code>long</code> which corresponds to what
 * we call a "willet" in honour of J.R. Willett in the same fashion as the
 * smallest Bitcoin unit is called a "satoshi".
 * </p>
 *
 * <p>
 * Note: J.R. Willett spells his name with two t's. OmniJ is currently using a
 * single t misspelling. This should probably be fixed. See
 * https://github.com/OmniLayer/OmniJ/issues/142
 * </p>
 *
 * <p>
 * The constructors are <code>protected</code> and instances should be created
 * with the <code>of()</code> static methods which can take either
 * <code>BigDecimal</code> or <code>long</code> values as parameters.
 * </p>
 *
 * <p>
 * The various *<code>value()</code> methods of <code>Number</code> and
 * <code>NumberValue</code> will return values as used in the Omni Protocol
 * Specification, which means that <b>for divisible tokens the values will be
 * treated a decimal values and methods returning integer types will be throw
 * exceptions if their is a fractional component that would be truncated.</b>
 * </p>
 *
 * <p>
...
public abstract class OmniValue extends NumberValue {
	...
	// Willet max/min values, same as max/min for indivisible, but different than
	// for divisible
	...
	/**
	 * Default Constructor Used only by subclasses using internal (willets) format
	 *
	...
	public static OmniValue of(BigDecimal amount, PropertyType type) {
		return type.equals(PropertyType.DIVISIBLE) ? OmniDivisibleValue.of(amount)
				: OmniIndivisibleValue.of(amount.longValueExact());
	}

	public static OmniValue of(long amount, PropertyType type) {
		return type.equals(PropertyType.DIVISIBLE) ? OmniDivisibleValue.of(amount) : OmniIndivisibleValue.of(amount);
	}

	public static OmniValue ofWillets(long amount, PropertyType type) {
		return type.equals(PropertyType.DIVISIBLE) ? OmniDivisibleValue.ofWillets(amount)
				: OmniIndivisibleValue.ofWillets(amount);
	}
	...
	/**
	 * <p>
	 * Make sure a BigInteger value is a valid Omni "number of coins" (willets)
	 * value
	 * </p>
	 *
	 * @param willets "number of coins" (willets) value to check
	 * @throws ArithmeticException if less than minimum or greater than maximum
	 *                             allowed value
	 */
	 ...
	 *
	 * <p>
	 * Note: Since any positive long is valid, we just need to check that it's not
	 * less than MIN_VALUE
	 * </p>
	 *
