...
public class TbProductServiceImpl extends ServiceImpl<TbProductMapper, TbProduct> implements ITbProductService {
	...
	public AppResultInfo<?> buyProduct(Integer userId, Integer productId) {
		...
		synchronized (newtbUserAssets) {
			...
			article.setState(1);
			article.setContent(user.getPhone().substring(0, 3) + "******" + user.getPhone().substring(9, user.getPhone().length()) );
			article.setClickNum(tbProduct.getProductType());
	...
	private void addUserAssetsAndBuyLog(Integer userId, TbUserAssets newtbUserAssets, TbProduct tbProduct) {
		// 用户余额
		Double balance = Double.parseDouble(BigDecimalUtils.subtract(newtbUserAssets.getCoin().toString(),
		...
		newtbUserAssets.setLimitCoin(Double.parseDouble(BigDecimalUtils.add(outNumber.toString(),
	 ...
	 * @param percent    返利点
	 */
	private void addProfit(Integer rebateUser, Integer userId, Double price, Float percent) {
		...
		synchronized (tbuserassets) {
			...
			entity.setAmount(value.doubleValue());
			Double t_value = Double.parseDouble(BigDecimalUtils.add(tbuserassets.getGiftCoin().toString(),
					value.toString()));
	 ...
	 * @return
	 */
	private AppUser getInviterAndAddExperience(AppUser user, Double price, Integer type, int first,int grade) {
